            ...
            ajax.jsonRpc("/get/user/email", 'call', {'mobile':mobile, 'login':login})
                .then(function (data) {
                    if (data.status == 1) {
                        ...
                        $('#wk_error').remove();
                        if (mobile) {
                            $('label[for=login], input#login').attr('value',data.login);
                        } else {
                            $('label[for=mobile], input#mobile').attr('value',data.mobile);
                        }
                        $(".field-password").show();
                        $(":submit").show();
                        $(".wk_next_btn").hide();
        ...
        if (loginType) {
            ajax.jsonRpc("/get/user/email", 'call', {'mobile':mobile, 'login':login})
                .then(function (data) {
                    if (data.status == 1) {
                        if (mobile) {
                            $('input#login').attr('value',data.login);
                        } else {
                            $('input#mobile').attr('value',data.mobile);
                        }
                    }
                });
        }
